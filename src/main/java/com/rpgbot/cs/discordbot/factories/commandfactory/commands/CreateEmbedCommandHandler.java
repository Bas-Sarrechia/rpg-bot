	...
	public void handle(MessageCreateEvent messageCreateEvent, String message) {
		...
		if (message.split(" ").length > 2) {
			...
			try {
				...
				commandService.lookupEmbedCommand(command);
				// sends error embed
				...
				// success!
				messageCreateEvent.getChannel().sendMessage(embedGeneratorFactory.get(EmbedType.SUCCESS).build("Embed added: " + command));
			}
		} else {
